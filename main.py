...
for ssp in ssps:
    ...
    for model in models:
        ...
        for real in reals:
            ...
            # Regrid, annualize, and fill in missing values
            if ssp == "ssp119" :
                ...
                plt.close(fig)
                print(f"DONE: {ssp}, model {model}, realization {real}")
                # plt.show()
            ...
            # Output
            # output_loc = (
            #     f"{mmrpm2p5_path}/{ssp}/mmrpm2p5/{model}/{real}/{glabel[0]}/annual.nc"
            # )
            # print(output_loc)
            # annual_ds.to_netcdf(output_loc)
