    ...
    for ssp in ssps:
        ...
        for year_ind in range(len(years)):
            ...
            disease_files = [x for x in mortality_files if
                             disease in x and str(year) in x and not any(exclude in x for exclude in excludes)]
...
def all_years():
    ...
    pool.map(process1, [disease for disease in diseases])
    pool.close()
    pool.join()
...
def avg():
    pool = multiprocessing.Pool(4)
    pool.map(process2, [disease for disease in diseases])
    pool.close()
    pool.join()
